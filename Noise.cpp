...
pair PerlinNoise::randCorner(int x, int y)
{
	//хэш-функция
...
double PerlinNoise:: Dot(pair a, pair b) //Скалярное произведение
{
...
double PerlinNoise::Noise(float fx, float fy)
{
	...
	pair tLG = this->randCorner(left, top);
	pair tRG = this->randCorner(left + 1, top);
	pair bLG = this->randCorner(left, top + 1);
	pair bRG = this->randCorner(left + 1, top + 1);
	pair distanceToTopLeft = { pointInQuadX, pointInQuadY };
	pair distanceToTopRight = { pointInQuadX - 1, pointInQuadY };
	pair distanceToBottomLeft = { pointInQuadX, pointInQuadY - 1 };
	pair distanceToBottomRight = { pointInQuadX - 1, pointInQuadY - 1 };
...
std::vector<int> PerlinNoise::getColor(int id, int initCol)
{
	...
	switch (id)
	{
	case 0: return { initCol % 218, initCol, 232 }; //Cloud
	case 2: return { 7, initCol >= 120 ? 120 : initCol < 40 ? 40 : initCol, 0 }; //Khaki
	case 3: return { initCol - 40, initCol + 40, initCol };  //Malachite
	default: return { initCol, initCol, initCol }; //Cloud

	}
	//default: return { initCol + 150, initCol + 60, initCol + 200 }; //idk
